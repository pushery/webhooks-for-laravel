...
- **A real PostgreSQL 18 server.** Note this is stricter than what the package *supports*
  (PostgreSQL 13+): the suite asserts its own floor and refuses to run below it, so a green
  local run always means the same thing as a green CI run. There is no SQLite fallback: the
  delivery log uses
