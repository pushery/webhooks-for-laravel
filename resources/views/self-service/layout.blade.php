{{-- Default full-page layout for the self-service endpoint portal.
...
<head>
    ...
    <title>{{ $title ?? __('webhooks::self-service.title') }}</title>
...
<body class="bg-[var(--color-wk-bg)] text-[color:var(--color-wk-text)] antialiased">
    {{-- WCAG 2.4.1 (Bypass Blocks): the first tab stop skips the page chrome straight
         into the content region, on every navigation. --}}
    <x-wirekit::skip-link target="wh-main" :label="__('webhooks::self-service.a11y.skip_to_content')" />
