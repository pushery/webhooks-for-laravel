...
<div class="wh-dash-activity" wire:key="hourly-activity" wire:poll.{{ config('webhooks.dashboard.poll_interval', '30s') }}>
    <x-wirekit::card>
        <x-wirekit::card.header>
            <x-wirekit::heading :level="2" size="sm">{{ __('webhooks::dashboard.activity.title') }}</x-wirekit::heading>
        </x-wirekit::card.header>
        <x-wirekit::card.body>
            @if ($rows->isEmpty())
                    ...
                    @foreach ($rows as $row)
                        ...
                        <div
                            ...
                            {{-- Four trans_choice calls rather than one __() with four numbers.
                                 ...
                                 `trans_choice` takes one count per string, and there are four — so
                                 a single sentence froze all four adjectives in the plural and every
                                 bucket holding exactly one delivery announced "1 livrées". That is
                                 the common bucket: a thirty-day window renders up to 720 of them
                                 and most are sparse.

                                 Joined here rather than in the lang file because the pieces are a
                                 LIST — its order is not grammar, so nothing a translator needs is
                                 taken away by assembling it. --}}
                            aria-label="{{ $hour.': '.implode(', ', [
                                trans_choice('webhooks::dashboard.a11y.hour_summary.total', $total, ['count' => $total]),
                                trans_choice('webhooks::dashboard.a11y.hour_summary.delivered', $delivered, ['count' => $delivered]),
                                trans_choice('webhooks::dashboard.a11y.hour_summary.pending', $pending, ['count' => $pending]),
                                trans_choice('webhooks::dashboard.a11y.hour_summary.failed', $failed, ['count' => $failed]),
                            ]) }}"
                        ...
                        >
                            <div class="flex h-full flex-col justify-end">
                                <div class="flex w-full flex-col-reverse overflow-hidden rounded-t-[var(--radius-wk-sm)]" style="height: {{ max($barHeight, $total > 0 ? 2 : 0) }}%;">
                                    @if ($delivered > 0)
                                        <div class="bg-[var(--color-wk-success)]" style="height: {{ round($delivered / max($total, 1) * 100) }}%"></div>
                                    @endif
                                    @if ($pending > 0)
                                        <div class="bg-[var(--color-wk-warning)]" style="height: {{ round($pending / max($total, 1) * 100) }}%"></div>
                                    @endif
                                    @if ($failed > 0)
                                        <div class="bg-[var(--color-wk-danger)]" style="height: {{ round($failed / max($total, 1) * 100) }}%"></div>
                                    @endif
                                </div>
                            </div>
                        </div>
